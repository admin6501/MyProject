...
create_menu() {
    ...
    curl -s -X POST "$BASE_URL/sendMessage" -d chat_id="$chat_id" -d text="$text" -d reply_markup="$keyboard" -d parse_mode="HTML"
}

# مثال استفاده از توابع
buttons=$(create_button "Button 1" "callback_1"),$(create_button "Button 2" "callback_2")
create_menu "$ADMIN_CHAT_ID" "Choose an option:" "$buttons"
