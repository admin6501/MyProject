...
# - No Docker / No docker-compose / No Python venv
# - Installs Python deps system-wide
# - Installs Xray via official XTLS installer (latest)
# - Creates systemd services: xray, omidvpn-panel, omidvpn-sync.timer

# --- force bash (prevents "set: Illegal option -o pipefail" when run with sh) ---
...
XRAY_BIN="/usr/local/bin/xray"  # will be auto-detected after official install
...
install_xray(){
  ...
  # Official installer (recommended by you)
  if ! bash -c "$(curl -fsSL https://github.com/XTLS/Xray-install/raw/main/install-release.sh)" @ install; then
    die "Xray official installer failed. Check connectivity to github.com."
  fi
  ...
  # Ensure our config exists (we manage /etc/xray/config.json)
  mkdir -p "$XRAY_ETC"
...
JSON
  ...
  # Ensure service exists & running (installer usually creates xray.service)
  systemctl daemon-reload || true
  systemctl enable --now xray || true
  systemctl is-active --quiet xray || die "xray service failed to start. Run: journalctl -u xray -e --no-pager"
}
...
final_print(){
  ...
  echo "  (مسیرها باید با / شروع شوند و فایل‌ها قابل خواندن باشند)"
  echo
  echo "Services:"
