#!/usr/bin/env bash
# Omid VPN (امید وی‌پی‌ان) Installer
# IMPORTANT: run with bash -> sudo bash install.sh
# No Docker / No docker-compose / No Python venv. Installs Python libs system-wide.

# If someone runs with sh, re-exec with bash to avoid "pipefail illegal option" etc.
...
prompt_settings(){
  ...
  # IMPORTANT: With `set -e`, read can sometimes return non-zero in some terminals.
  # So we use `|| true` to prevent sudden exit.
  read -r -p "Panel port [${DEFAULT_PANEL_PORT}]: " PANEL_PORT || true
  ...
  read -r -p "Admin username [${DEFAULT_ADMIN_USER}]: " ADMIN_USERNAME || true
  ADMIN_USERNAME="${ADMIN_USERNAME:-$DEFAULT_ADMIN_USER}"
  [[ -z "$ADMIN_USERNAME" ]] && die "نام کاربری نمی‌تواند خالی باشد."

  read -r -s -p "Admin password [${DEFAULT_ADMIN_PASS}]: " ADMIN_PASSWORD || true
  echo
...
install_python_libs(){
  ...
  # Ubuntu 24.04+ can block pip installs into system site-packages (PEP 668).
  # Using --break-system-packages avoids abrupt failure.
...
class Client(Base):
    ...
    ip_limit = Column(Integer, nullable=True)        # placeholder
...
TRANSLATIONS = {
  "fa": {
    ...
    "stream_json":"Stream JSON (ترنسپورت/حمل‌ونقل)"
  },
  "en": {
    "brand":"Omid VPN",
    "dashboard":"Dashboard",
    ...
    "stream_json":"Stream JSON (Transport)"
  }
}
