...
# اگر کسی با sh اجرا کرد، خودکار با bash اجرا شود
...
# خواندن امن ورودی‌ها از /dev/tty (حل مشکل Termius/SSH)
prompt_settings(){
  echo "=== Omid VPN (امید وی‌پی‌ان) Installer ==="
  echo

  local tty="/dev/tty"
  [[ -r "$tty" && -w "$tty" ]] || die "Cannot access /dev/tty. لطفاً با ترمینال واقعی اجرا کنید."

  # Panel port
  printf "Panel port [%s]: " "${DEFAULT_PANEL_PORT}" >"$tty"
  IFS= read -r PANEL_PORT <"$tty" || PANEL_PORT=""
  ...
  # Admin username
  printf "Admin username [%s]: " "${DEFAULT_ADMIN_USER}" >"$tty"
  IFS= read -r ADMIN_USERNAME <"$tty" || ADMIN_USERNAME=""
  ADMIN_USERNAME="${ADMIN_USERNAME:-$DEFAULT_ADMIN_USER}"
  [[ -z "$ADMIN_USERNAME" ]] && die "نام کاربری نمی‌تواند خالی باشد."

  # Admin password (silent)
  printf "Admin password [%s]: " "${DEFAULT_ADMIN_PASS}" >"$tty"
  IFS= read -r -s ADMIN_PASSWORD <"$tty" || ADMIN_PASSWORD=""
  printf "\n" >"$tty"
...
main(){
  ...
  # برای ترمینال‌هایی مثل Termius که stdin را گاهی می‌برند:
  exec </dev/tty
